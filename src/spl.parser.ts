...
import {Let, SimpleTree} from './tools';
...
export class FunDecl extends ParserRule{
	...
	getReturnPaths() {
		let compose = (A: {maybe: ParserRule[], concl: ParserRule[]}, B: {maybe: ParserRule[], concl: ParserRule[]}) => {
			if(A.concl.length && B.concl.length)
				return {maybe: [...A.maybe, ...B.maybe], concl: [...A.concl, ...B.concl]};
			if(A.concl.length == B.concl.length)
				return undefined;
			return {maybe: [...A.maybe, ...B.maybe, ...A.concl, ...B.concl], concl: []};
		}
		let get = (o: Assign | Call | If | While | Ret | Stmt[] | Block | Stmt) : {maybe: ParserRule[], concl: ParserRule[]} | undefined => {
			if(o instanceof Assign || o instanceof Call)
				return undefined;
			if(o instanceof Ret)
				return {maybe: [], concl: [o]};
			if(o instanceof Array){
				...
				let x = <{maybe: ParserRule[], concl: ParserRule[]}[]>r.filter(o => o);

				if(x[x.length-1].concl.length)
					return {maybe: x.map(o => o.maybe).reduce((p,c) => p.concat(c), []), concl: x[x.length-1].concl};
				return {maybe: x.map(o => [...o.maybe, ...o.concl]).reduce((p,c) => p.concat(c), []), concl: []};
			}
			...
			if(o instanceof If){
				let branches = [];
				let branch1 = get(o.block);
				let branch2 = o.else ? get(o.else.block) : undefined;
				branch1 && branches.push(branch1);
				branch2 && branches.push(branch2);
				return  branches.length == 0 ? undefined :
						branches.length == 1 ? {maybe: [...branches[0].maybe, ...branches[0].concl], concl: []}
											 : compose(branches[0], branches[1]);
			}
			return Let(get(o.block), o => o ? ({maybe: [...o.maybe, ...o.concl], concl: []}) : undefined);
		}
		return get(this.Stmt);
	}
	_getType(ctx: T.Context) : T.Type {
		let argsNames = this._args ? this._args.args : [];
		if(argsNames.length != new Set(argsNames).size)
			throw (<FArgs>this._args).name.error(": duplicate name in parameters");
		let inputTypes = argsNames.map(n => new T.VariableType(this, n.content));

		// local context of function
		let funCtx = ctx.child();
		this.ctx = funCtx;
		inputTypes.forEach(i => funCtx.set(i.name, i, true));

		// we compute types
		this.variables.forEach(o => o.getType(funCtx));
		this.Stmt.forEach(o => o.getType(funCtx));
		funCtx.getTypes().forEach(o => o instanceof T.VariableType && o.seal());

		// look for return statements
		let R = this.getReturnPaths();
		if(!R || R.concl.length==0)
			throw this.error("Not all paths returns value");
		let lR_ = [...R.maybe, ...R.concl];

		let lR = lR_.filter(o => !(o.getType(funCtx) instanceof T.NotResolvedYet));
		if(!lR.length)
			throw this.error("Unsolvable recursive pattern detected.");

		let firstReturn = lR[0];
		let firstReturnType = firstReturn.getType(funCtx);
		let error = lR.slice(1).find(o => !o.getType(funCtx).unifyWith(firstReturnType, T.ForceSealState.Nothing));
		if(error)
			throw error.error('Return statements of different types');

		let funType = ctx.get(this.name.content);
		if(funType && !(funType instanceof T.FunctionType))
			throw this.error("This function name already exists (can be a function or a variable)");
		else if(!funType || (funType instanceof T.NotResolvedYet)){
			funType = new T.FunctionType(this, []);
			ctx.set(this.name.content, funType);
		}
		if(!ctx.functionSealed)
			(<T.FunctionType>funType).funDefs.push(new T.FunDefType(this, inputTypes, firstReturnType));

		return funType;
	}
	SSM() : string[]{
		let prog = [this.name.content+': link 1', 'ldl 0', 'lda 1', 'stl 1'];
		let vars = this.variables.map(o => o.SSM()).reduce((p,c) => p.concat(c), []);
		let stmts = this.Stmt.map(o => o.SSM()).reduce((p,c) => p.concat(c), []);
		return [...prog, ...vars, ...stmts];
	}
}
...
export class Type extends ParserRule{
	@SPL_Parser.addStep() 		type: LexSPL.BasicType | TupleType | ListType | LexSPL.Id;
	_getType(ctx: T.Context) : T.Type {
